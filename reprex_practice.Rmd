...
mack_creek_lengths <- mack_creek_vertebrates %>% 
  ...
  filter(section == "CC") %>% 
  #mutate(across(where(is.character, tolower)))%>% - I fixed this line by closing the where() around is.character
...
#geom_point(aes(shape = 12), color = "purple") - I fixed this like by taking out aes()
+
         theme_minimal() +
